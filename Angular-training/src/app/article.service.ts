...
export class ArticleService {
	...
	allLengthsUrl = "http://localhost:9087/user/lengths";
	...
	getLength():Observable<Length>{
		return this.http.get(this.allLengthsUrl).map(this.extractData).catch(this.handleError);
		
	}
	//Fetch max and min
/*	getValid(): Observable<Length> {
        return this.http.get(this.allLengthsUrl)
		   		.map(this.extractData)
		        .catch(this.handleError);
